...
def _location_to_path(host: str, loc: str) -> str:
    ...
    if parsed.scheme or parsed.netloc:
        return parsed.path
    # A relative Location without a leading slash (e.g. "r/dashboard/self")
    # must be treated as absolute-from-root; otherwise it is concatenated
    # onto the host and parsed as part of the netloc, dropping the
    # base-path segment and corrupting base-path inference.
    rel = loc if loc.startswith("/") else f"/{loc}"
    return urllib.parse.urlparse(f"https://{host}{rel}").path
        ...
        for scheme in ("https", "http"):
            for probe in probes:
                ...
                if result is not None:
                    # Cache the discovered base path (including an empty
                    # string for the 200-OK "no base path" case) so we do
                    # not re-probe the network on subsequent lookups.
                    _BASE_PATH_CACHE[host] = result
                    return result
