...
def _backoff_delay(attempt: int, base: float = 0.5, cap: float = 6.0) -> float:
    ...
    delay = float(min(base * (2 ** max(0, attempt - 1)), cap))
    jitter = float(random.uniform(0.0, delay / 2.0))  # noqa: S311
...
def _is_transient_exception(exc: BaseException) -> bool:
    ...
    if isinstance(exc, socket.timeout | TimeoutError | ConnectionResetError | ConnectionAbortedError | BrokenPipeError):
        return True
    if isinstance(exc, urllib.error.URLError):
        ...
        return any(s in msg for s in _TRANSIENT_ERR_SUBSTRINGS)
    msg = str(exc).lower()
    return any(s in msg for s in _TRANSIENT_ERR_SUBSTRINGS)
...
class GerritRestClient:
    ...
    def put(self, path: str, data: Any | None = None) -> Any:
        ...
        return self._request_json("PUT", path, data=data)

    # Internal helpers

    def _request_json(self, method: str, path: str, data: Any | None = None) -> Any:
        """Perform a JSON request with retries."""
        if not path:
        ...
        for attempt in range(1, self._attempts + 1):
            try:
                ...
                if _is_transient_http_status(status) and attempt < self._attempts:
                    delay = _backoff_delay(attempt)
                    log.warning("%s; retrying in %.2fs", msg, delay)
                    _sleep(delay)
                    continue
                ...
                last_exc = exc
                if _is_transient_exception(exc) and attempt < self._attempts:
                    delay = _backoff_delay(attempt)
                    log.warning("Gerrit REST %s %s transient error: %s; retrying in %.2fs", method, url, exc, delay)
                    _sleep(delay)
                    continue
        ...
        # Should not reach here; if it does, raise last exception
        msg_final = f"Gerrit REST {method} {url} failed after {self._attempts} attempts"
        raise GerritRestError(msg_final) from last_exc
...
def _sleep(seconds: float) -> None:
    # Isolated to ease patching in tests without importing 'time' at module level
    import time
