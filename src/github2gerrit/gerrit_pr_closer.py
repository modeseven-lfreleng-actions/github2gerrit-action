...
def extract_change_number_from_url(
    ...
    """
    # Pattern: https://host/c/project/+/number or https://host/infra/c/project/+/number
    # More flexible pattern to handle various Gerrit URL formats
    # Match: host, optional subpath(s), /c/, project path, then +/number
    # NOTE: Keep in sync with gerrit_pattern in cli.py:_parse_target_url
    pattern = r"https?://([^/]+)/(?:.*/)?c/[^+]+/\+/(\d+)"
    match = re.match(pattern, gerrit_change_url)
...
def close_github_pr_for_merged_gerrit_change(
    ...
    *,
    ...
    """
    Close a GitHub PR when its corresponding Gerrit change has been merged.

    This function:
    1. Extracts the GitHub PR URL from the commit's trailers
    2. Optionally verifies the Gerrit change was merged (not abandoned)
    3. Fetches the PR details from GitHub
    4. Validates the PR exists and matches the commit
    5. Displays the PR details
    6. Adds a comment explaining the closure
    7. Closes the PR
    ...
    Args:
        ...
        force: If True, close PR regardless of Gerrit change status

    Returns:
        True if PR was closed (or would be closed in dry-run), False otherwise

    Raises:
        GitHub2GerritError: If Gerrit change was abandoned and force=False
    """
    log.info("Processing Gerrit change: %s", commit_sha[:8])

    # If we have a Gerrit change URL and force is not set, verify it was merged
    if gerrit_change_url and not force:
        ...
        if status == "ABANDONED":
            ...
            raise GitHub2GerritError(
                ...
                message="Gerrit change was abandoned",
                details=gerrit_change_url,
            )
        elif status == "NEW":
            log.warning(
    ...
    try:
        ...
        # Unexpected errors - log as warning but continue processing
        log.warning(
            "Failed to close GitHub PR #%d: %s - skipping",
            pr_number,
            exc,
        )
        return False
...
def process_recent_commits_for_pr_closure(
    ...
    *,
    ...
    Args:
        ...
        progress_tracker: Optional progress tracker for display management
        force: If True, close PRs regardless of Gerrit change status
