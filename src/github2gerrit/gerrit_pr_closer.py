...
def _apply_pr_close_action(
    ...
    *,
    ...
    # Describe the change using its actual Gerrit status; "abandoned" is
    # only accurate for ABANDONED changes, whereas this path also handles
    # MERGED/NEW/UNKNOWN closures.
    status_label = {
        "ABANDONED": "abandoned ",
        "MERGED": "merged ",
        "NEW": "open ",
    ...
    close_message = (
        f"🛑 Closed pull request #{pr_number} with {status_label}"
...
def cleanup_closed_github_prs(
    ...
    *,
    ...
    try:
        ...
        log.warning(
            "Gerrit cleanup for closed GitHub PRs failed (non-fatal)",
            exc_info=True,
        )
        return 0
    else:
