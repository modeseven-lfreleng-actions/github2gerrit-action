...
class Orchestrator:
    ...
    def _resolve_gerrit_info(
        ...
        # The file's port counts only when the file named one; the
        # parser's default is not the file's word.
        file_port = gitreview.port if gitreview and gitreview.port_given else 0
        if gitreview and explicit_host is None:
            ...
            port = (
                (int(explicit_port) if explicit_port else 0)
                or file_port
                or inputs.gerrit_server_port
                or 29418
            )
        ...
        # The port follows its own tier: an explicit input, else the
        # file's, else a derived input, else the Gerrit default.
        port = (
            (int(explicit_port) if explicit_port else 0)
            or file_port
            or inputs.gerrit_server_port
