...
def test_gerrit_rest_parses_xssi_guard_and_json_success(
    ...
    # Avoid sleeping on retries (not needed in this test)
    monkeypatch.setattr("github2gerrit.gerrit_rest._sleep", lambda s: None, raising=True)
...
def test_commit_msg_hook_integrity_failure_due_to_small_size(
    ...
    # Provide a tiny payload (below 128 bytes) and missing shebang
    def _tiny(_: Path) -> bytes:
        return b"echo not a hook\n"

    fake_run = _mock_run_cmd_writer(_tiny)
    monkeypatch.setattr("github2gerrit.gitutils.run_cmd", fake_run, raising=True)
...
def test_commit_msg_hook_integrity_success_and_executable(
    ...
    # Provide a realistic payload with shebang and recognizable markers
    def _valid(_: Path) -> bytes:
        lines = [
            "#!/bin/sh\n",
    ...
    fake_run = _mock_run_cmd_writer(_valid)
    monkeypatch.setattr("github2gerrit.gitutils.run_cmd", fake_run, raising=True)
