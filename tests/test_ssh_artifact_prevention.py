...
def temp_workspace():
    ...
    with tempfile.TemporaryDirectory() as temp_dir:
        ...
        subprocess.run(["git", "init"], cwd=workspace, check=True, env=env)
        subprocess.run(
...
def test_git_operations_exclude_ssh_artifacts(temp_workspace):
    ...
    subprocess.run(
        ["git", "add", "test_file.txt"], cwd=temp_workspace, check=True, env=env
    )
    subprocess.run(
    ...
    subprocess.run(["git", "add", "."], cwd=temp_workspace, check=True, env=env)
