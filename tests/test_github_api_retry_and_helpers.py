...
def test_should_retry_with_tuple_isinstance() -> None:
    """Test that _should_retry works correctly with tuple isinstance syntax."""
    # Test with rate limit exception
    rate_limit_exc = ghapi.RateLimitExceededExceptionType()
    assert ghapi._should_retry(rate_limit_exc) is True
    ...
    assert ghapi._should_retry(github_exc) is True
    ...
    assert ghapi._should_retry(github_exc_403) is True

    # Test with non-retryable exception
    other_exc = ValueError("some other error")
    assert ghapi._should_retry(other_exc) is False
    ...
    assert ghapi._should_retry(github_exc_400) is False
